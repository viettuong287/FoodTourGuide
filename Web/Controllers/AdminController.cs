...
namespace Web.Controllers
{
    public class AdminController : Controller
    {
        ...
        public async Task<IActionResult> Dashboard(CancellationToken cancellationToken)
        {
            ...
            var devicesTask = _geoApiClient.GetActiveDevicesAsync(30, cancellationToken); // Thống kê thiết bị trong 30s qua

            await Task.WhenAll(businessesTask, stallsTask, languagesTask, narrationTask, devicesTask);
            ...
            var vm = new AdminDashboardViewModel
            {
                ...
                ActiveDevicesCount = (await devicesTask)?.Data?.ActiveCount ?? 0,
        ...
        public async Task<IActionResult> ResetDevice(string deviceId, CancellationToken cancellationToken = default)
        {
            ...
            var result = await _deviceApiClient.ResetDeviceAsync(deviceId, cancellationToken);
            if (result?.Success == true)
        ...
        public async Task<IActionResult> Devices(
            int page = 1, int pageSize = 30, string? search = null,
            CancellationToken cancellationToken = default)
        {
            var result = await _deviceApiClient.GetDevicesAsync(page, pageSize, search, cancellationToken);
            ViewBag.Search = search;
            return View(result?.Data ?? new Shared.DTOs.Common.PagedResult<Shared.DTOs.DevicePreferences.DevicePreferenceDetailDto>
            {
                Items = [],
                TotalCount = 0,
                Page = page,
