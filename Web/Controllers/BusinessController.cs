...
namespace Web.Controllers
{
    public class BusinessController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind(Prefix = "Create")] BusinessFormViewModel model, int page = 1, int pageSize = 10, string? search = null, CancellationToken cancellationToken = default)
        {
            ...
            {
                return await ReturnWithCreateError(model, page, pageSize, search, null, cancellationToken);
            }
            ...
            {
                return await ReturnWithCreateError(model, page, pageSize, search, result?.Error?.Message ?? "Tạo business thất bại.", cancellationToken);
            }
        ...
        public async Task<IActionResult> Update([Bind(Prefix = "Edit")] BusinessFormViewModel model, int page = 1, int pageSize = 10, string? search = null, CancellationToken cancellationToken = default)
        {
            if (model.Id == null)
            {
                return await ReturnWithEditError(model, page, pageSize, search, "Không tìm thấy business cần cập nhật.", cancellationToken);
            }

            if (!ModelState.IsValid)
            {
                return await ReturnWithEditError(model, page, pageSize, search, null, cancellationToken);
            }
            ...
            if (result?.Success != true)
            {
                return await ReturnWithEditError(model, page, pageSize, search, result?.Error?.Message ?? "Cập nhật business thất bại.", cancellationToken);
            }
        ...
        private async Task<BusinessManagementViewModel> BuildViewModel(int page, int pageSize, string? search, CancellationToken cancellationToken)
        {
        ...
        private async Task<IActionResult> ReturnWithCreateError(BusinessFormViewModel model, int page, int pageSize, string? search, string? errorMessage, CancellationToken cancellationToken)
        {
            var viewModel = await BuildViewModel(page, pageSize, search, cancellationToken);
        ...
        private async Task<IActionResult> ReturnWithEditError(BusinessFormViewModel model, int page, int pageSize, string? search, string? errorMessage, CancellationToken cancellationToken)
        {
            var viewModel = await BuildViewModel(page, pageSize, search, cancellationToken);
