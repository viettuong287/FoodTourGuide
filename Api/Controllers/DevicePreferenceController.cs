...
namespace Api.Controllers
{
    ...
    public class DevicePreferenceController : ControllerBase
    {
        ...
        {
            var query = _context.DevicePreferences
                .AsNoTracking()
            ...
            if (!string.IsNullOrWhiteSpace(search))
            {
                ...
                query = query.Where(x => 
                    ...
                    x.DeviceModel!.ToLower().Contains(s) || 
                    x.Manufacturer!.ToLower().Contains(s));
            }
            ...
            return this.OkResult(new PagedResult<DevicePreferenceDetailDto>
            {
                Items = items,
                TotalCount = total,
                Page = page,
        ...
        public async Task<IActionResult> Upsert([FromBody] DevicePreferenceUpsertDto request)
        {
            ...
            Console.WriteLine($"\n[DEVICE ACTIVITY] {now:HH:mm:ss} - DeviceId: {request.DeviceId} - Model: {request.DeviceModel}");
            ...
            if (language is null)
            {
                // Nếu không tìm thấy ngôn ngữ yêu cầu (có thể do mobile chưa sync xong), 
                // thử lấy ngôn ngữ active đầu tiên thay vì trả về BadRequest.
                language = await _context.Languages.AsNoTracking().FirstOrDefaultAsync(x => x.IsActive);
                
                if (language is null)
                    return this.BadRequestResult("Hệ thống chưa cấu hình ngôn ngữ nào.", "LanguageId");
            }
